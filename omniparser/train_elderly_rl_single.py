#!/usr/bin/env python3
"""
고령자 강화학습 에이전트 훈련 스크립트 (단일 데이터 폴더)

이 스크립트는 특정 UIUX 인증 데이터 폴더를 사용하여
...
def load_single_training_data(data_path):
    """특정 데이터 폴더에서 훈련 데이터를 로드합니다."""
    print(f"데이터 폴더에서 훈련 데이터 로드 중: {data_path}")
    ...
    try:
        ...
        if len(gaze_data) < 10:  # 최소 10개의 시선 데이터 포인트 필요
            print(f"❌ 시선 데이터가 너무 적습니다: {len(gaze_data)}개")
            return None
        ...
        if episode_data and len(episode_data['states']) > 0:
            ...
            return [episode_data]  # 리스트로 반환
        else:
            print(f"❌ 유효한 훈련 데이터가 없습니다")
            return None
...
def convert_to_training_data(scenario_data, gaze_data, click_events, timeline_data=None, performance_metrics=None):
    ...
    for i, gaze_point in enumerate(gaze_data):
        ...
        # 보상 생성 (고령자 특성 반영)
        reward = create_elderly_reward(gaze_point, click_events, scenario_data, i, gaze_data)
...
def create_elderly_reward(gaze_point, click_events, scenario_data, step_index, gaze_data=None):
    ...
    if step_index > 10 and gaze_data is not None:
        recent_positions = []
...
def main():
    """메인 실행 함수"""
    parser = argparse.ArgumentParser(description='고령자 강화학습 에이전트 훈련 (단일 데이터 폴더)')
    parser.add_argument('--data_path', type=str, 
                       default='../beameyetracker/kiosk_data/uiux_certification_20250712_180612',
                       help='훈련 데이터 폴더 경로')
    ...
    # 훈련 데이터 로드
    training_data = load_single_training_data(args.data_path)
    
    if not training_data:
        print("❌ 훈련 데이터를 찾을 수 없습니다.")
        print(f"데이터 경로를 확인해주세요: {args.data_path}")
        return
