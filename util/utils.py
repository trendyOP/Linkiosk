...
def get_caption_model_processor(model_name, model_name_or_path="Salesforce/blip-image-captioning-base", device=None):
    if not device:
...
def get_parsed_content_icon(filtered_boxes, starting_idx, image_source, caption_model_processor, prompt=None, batch_size=8):
    ...
    for i in range(0, len(croped_pil_image), batch_size):
        ...
        if model.device.type == 'cuda':
            inputs = processor(images=batch, text=[prompt]*len(batch), return_tensors="pt", do_resize=False).to(device)
        else:
        ...
        if 'florence' in model.config.name_or_path:
            ...
            generated_ids = model.generate(**inputs, max_length=30, num_beams=1, no_repeat_ngram_size=2, early_stopping=True, num_return_sequences=1) # temperature=0.01, do_sample=True,
        generated_text = processor.batch_decode(generated_ids, skip_special_tokens=True)
        generated_text = [gen.strip() for gen in generated_text]
        generated_texts.extend(generated_text)
    print("[INFO] BLIP captioning finished in", time.time() - start, "seconds")
